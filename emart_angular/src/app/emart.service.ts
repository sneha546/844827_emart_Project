...
export class EmartService {
  ...
  getAllItems(): any {
    return this.Http.get("http://localhost:8087/item/all");
  ...
  getItem(itemId: String) {
    ...
    return this.Http.get("http://localhost:8087/item/" + itemId);
  }
  ...
  addBill(todayDate: Date, total: number) {
    ...
    this.cartItems = [];
    allBillDetails = [];
    return this.Http.post("http://localhost:8087/emart/bill", bill);
  }

  /*To get buyer details */
  getBuyer() {
    return this.Http.get("http://localhost:8087/emart/buyer/" + this.currentBuyer.buyerId);
  }
  ...
  validateBuyer(user: string, password: string) {
    ...
    return this.Http.get("http://localhost:8087/emart/validate", { headers });
  }
}
